...
def att_positivi():
    ...
    plt.legend([l1], ["{}: {}".format(date[-1].strftime("%d-%h"), raggruppati['totale_positivi'].tail(1).values[0])])
    plt.grid(alpha = 0.5)
    ...
    for regione in dati_regione['denominazione_regione'].unique():
        ...
        plt.grid(alpha = 0.5)
        plt.legend([l1], ["{}: {}".format(date[-1].strftime("%d-%h"), per_regioni.tail(1).values[0])])
