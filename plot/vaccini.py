...
def vaccini_fasce():
   ...
   df_fasce = df.groupby(['fascia_anagrafica']).sum()[['prima_dose', 'seconda_dose', 'dose_aggiuntiva', 'pregressa_infezione']]
   ...
   ax.bar(df_fasce.index, df_fasce['dose_aggiuntiva'], label='dose aggiuntiva', color = '#0061B6')
...
def vaccini_fasce_perc():
   ...
   perc_agg = df_fasce['dose_aggiuntiva'].sum()/df_fasce['massimi'].sum()*100
   ...
   df_fasce['dose_aggiuntiva'] = df_fasce['dose_aggiuntiva']/df_fasce['massimi']*100
   ...
   ax.bar(df_fasce.index, df_fasce['dose_aggiuntiva'], label=f'dose aggiuntiva ({round(perc_agg, 2)}%)', color = '#0061B6')
...
def vaccini():
   ...
   raggruppati['dose_aggiuntiva_ma'] = (raggruppati['dose_aggiuntiva']).rolling(window = 7).mean()
   ...
   plt.plot(date, raggruppati['dose_aggiuntiva'], color='#0061B6', alpha = 0.2)
   ...
   lg = plt.legend([l1, l2, l3, l4], ["Prima dose: {}".format(round(raggruppati['prima_dose'].tail(1).values[0],2)), 
                           ...
                           "Dose aggiuntiva: {}".format(round(raggruppati['dose_aggiuntiva'].tail(1).values[0],2)),
                           "Tot. somministrazioni: {} mln".format(round((raggruppati['prima_dose'].sum() + raggruppati['seconda_dose'].sum() + raggruppati['pregressa_infezione'].sum() + raggruppati['dose_aggiuntiva'].sum())/1e+06,3))])
...
def vaccini_cum():
   ...
   raggruppati['dose_aggiuntiva'] = (raggruppati['dose_aggiuntiva'].cumsum()/pop_ita) * 100
