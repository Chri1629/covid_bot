...
def positivi_tamponi():
    ...
    #raggruppati['rapporto_totale'] = raggruppati['nuovi_positivi']/raggruppati['tamponi']*100
    ...
    for regione in dati_regione['denominazione_regione'].unique():
        ...
        plt.hlines(y = per_regioni['rapporto'].tail(1), xmin = 0, xmax=max(date), 
        label = "Ultimo valore {}%".format(per_regioni['rapporto'].tail(1)))
        plt.xlim(left = 0)
        plt.ylim(bottom = 0)
        ...
        plt.xticks(size = 10)
        plt.yticks(size = 10)
        plt.title("Nuovi rapporto al giorno in {}".format(regione), size = 15)
        plt.grid()
        plt.legend()
        plt.show()
        fig.savefig("pics/rapporto_{}.png".format(regione), dpi = 100)
